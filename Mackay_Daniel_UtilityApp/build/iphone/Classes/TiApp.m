...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kMackay_Daniel_UtilityAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kMackay_Daniel_UtilityAppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Mackay_Daniel_UtilityApp app using the node.js CLI
