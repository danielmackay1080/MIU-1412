...
NSString * Mackay_Daniel_UtilityApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Mackay_Daniel_UtilityAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mackay_daniel_utilityapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Mackay_Daniel_UtilityApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Mackay_Daniel_UtilityApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mackay_daniel_utilityapp = [[Mackay_Daniel_UtilityAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mackay_daniel_utilityapp];
	
	NSString *_mackay_daniel_utilityappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mackay_daniel_utilityappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mackay_daniel_utilityapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Mackay_Daniel_UtilityApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mackay_daniel_utilityapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.mackay_daniel_utilityapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
