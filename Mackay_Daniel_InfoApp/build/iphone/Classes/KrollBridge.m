...
NSString * Mackay_Daniel_InfoApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Mackay_Daniel_InfoAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mackay_daniel_infoapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Mackay_Daniel_InfoApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Mackay_Daniel_InfoApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mackay_daniel_infoapp = [[Mackay_Daniel_InfoAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mackay_daniel_infoapp];
	
	NSString *_mackay_daniel_infoappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mackay_daniel_infoappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mackay_daniel_infoapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Mackay_Daniel_InfoApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mackay_daniel_infoapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.mackay_daniel_infoapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
